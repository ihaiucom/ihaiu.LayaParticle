...
export default class ParticleTemplateWebGL extends ParticleTemplateBase
{
    ...
    protected initialize(): void
    {
        ...
        let bufi = 0;
        let bufStart = 0;
        for(let i = 0; i < this.settings.maxPartices; i ++)
        {
            let random = Math.random();
            let cornerYSegement = this.settings.textureCount ? 1.0 / this.settings.textureCount : 1.0;
            let cornerY = NaN;
    ...
    {
        let epsilon = 0.0001;
        let particleDuration = this.settings.duration;
